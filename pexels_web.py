...
def run():
    search = input(Style.RESET_ALL + 'Search for free photos(The search must be in English): ')
    url = '{}{}'.format(URL, search)
    headers = {'User-Agent': 'Mozilla/5.0'}
    web = requests.get(url, headers=headers)
    soup = BeautifulSoup(web.content, 'html.parser')
    urls_images = soup.find_all('img', attrs={'class': 'photo-item__img'})
...
if __name__ == '__main__':
    init()
    print(Fore.GREEN + 'WELCOME TO MY PROGRAM TO DOWNLOAD IMAGES')
    run()
